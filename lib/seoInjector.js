...
function resolveOgImageUrl(rawValue, canonicalSiteUrl) {
  ...
  try {
    ...
    if (isBlockedDomain) {
      ...
      // Any other domain (external CDN, etc.) is allowed.
      // We previously restricted this to the canonical domain, but have relaxed it
      // to support the "External URL" dashboard feature.
      if (!isCorrectDomain) {
        issuesFound.push(`External domain detected: "${hostname}". Proceeding as external asset.`);
      }
    }
  } catch (_) {
