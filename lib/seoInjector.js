...
function buildSeoTags(content, lang = 'ar') {
  ...
  // Sanitize legacy domain: if a stored absolute URL references the old
  // comprehensivecancercenter.com domain, strip it back to a path so it gets
  // re-rooted to the correct SITE_URL (waleedarafat.org) below.
  if (ogImageUrl && typeof ogImageUrl === 'string' && ogImageUrl.includes('comprehensivecancercenter.com')) {
    try {
      const parsed = new URL(ogImageUrl);
      ogImageUrl = parsed.pathname; // e.g. /uploads/img-1776529742903-bmzatu.jpg
    } catch (_) {
      ogImageUrl = '/uploads/seo-og-image.jpg'; // fallback if URL is malformed
    }
  }
...
function injectSeoTagsIntoHtml(htmlContent, seoTags, rawJsonLd = null, hreflangLinks = []) {
  ...
  // This is critical because some social scrapers (Facebook/WhatsApp) only parse 
  // the first 50KB-100KB. Large inline CSS can push meta tags out of the parseable range.
